...
class sqliteparse(KaitaiStruct):
    """sqliteparse is a popular serverless SQL engine, implemented as a library
    ...
    def _read(self):
        ...
        self.write_version = KaitaiStream.resolve_enum(sqliteparse.Versions, self._io.read_u1())
        self.read_version = KaitaiStream.resolve_enum(sqliteparse.Versions, self._io.read_u1())
        ...
        self.text_encoding = KaitaiStream.resolve_enum(sqliteparse.Encodings, self._io.read_u4be())
        ...
        self.root_page = sqliteparse.BtreePage(self._io, self, self._root)
    ...
    class BtreePage(KaitaiStruct):
        ...
        def _read(self):
            ...
            for i in range(self.num_cells):
                self.cells[i] = sqliteparse.RefCell(self._io, self, self._root)
    ...
    class CellIndexLeaf(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.payload = sqliteparse.CellPayload(_io__raw_payload, self, self._root)
    ...
    class CellPayload(KaitaiStruct):
        ...
        def _read(self):
            ...
            self.column_serials = sqliteparse.Serials(_io__raw_column_serials, self, self._root)
            self.column_contents = [None] * (len(self.column_serials.entries))
            for i in range(len(self.column_serials.entries)):
                self.column_contents[i] = sqliteparse.ColumnContent(self.column_serials.entries[i], self._io, self, self._root)
    ...
    class RefCell(KaitaiStruct):
        ...
        def body(self):
            ...
            if _on == 13:
                self._m_body = sqliteparse.CellTableLeaf(self._io, self, self._root)
            elif _on == 5:
                self._m_body = sqliteparse.CellTableInterior(self._io, self, self._root)
            elif _on == 10:
                self._m_body = sqliteparse.CellIndexLeaf(self._io, self, self._root)
            elif _on == 2:
                self._m_body = sqliteparse.CellIndexInterior(self._io, self, self._root)
            self._io.seek(_pos)
            return self._m_body if hasattr(self, '_m_body') else None
...
data = sqliteparse.from_file("with1.db")
print(data.len_page)
print(data.sqlite_version_number)